...
--이름을 글자길이의 50%만 출력하고 나머지는 '*'로 표시하라
...
--부서를 1차 분류하고 부서내 담당업무를 2차 분류하여 사원수, 급여의 합계와 평균을 구하라
